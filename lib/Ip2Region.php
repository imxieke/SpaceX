 ...
 * Algorithm: binary, b-tree, memory
...
class Ip2Region
{
    ...
    */
    ...
    {
        if ( ! file_exists($ip2regionFile)) throw new \Exception("Ip2Region File Not Found");
        $this->dbFile = $ip2regionFile;
    }

    /**
    * @param $method string btreeSearch binarySearch memorySearch
    * @return json
    */
    public function get($ip = '' ,$method = 'btreeSearch')
    {
        $data = $this->$method($ip);
        $data = explode('|', $data['region']);
        $data['0'] == '0' ? $ipinfo['country']   = '' : $ipinfo['country']    = $data['0'] ;
        $data['2'] == '0' ? $ipinfo['province']  = '' : $ipinfo['province']   = $data['2'] ;
        $data['3'] == '0' ? $ipinfo['city']      = '' : $ipinfo['city']       = $data['3'] ;
        $data['4'] == '0' ? $ipinfo['isp']       = '' : $ipinfo['isp']        = $data['4'] ;
        return $ipinfo;
