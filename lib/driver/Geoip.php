...
/** 
* Query Ipinfo from Maxmind Geolite
*/
class Geoip
{
    private $asnFile;
    private $cityFile;
    function __construct($mmdbFile = [])
    {
        if ( ! empty($mmdbFile)) {
            if (  ! file_exists( $mmdbFile['asn'] ) )
            {
                throw new Exception("Maxmind ASN Data Not Found" . $mmdbFile['asn'] );
            }
            elseif ( ! file_exists( $mmdbFile['city'] ))
            {
                throw new Exception("Maxmind City Data Not Found" . $mmdbFile['city']);
            }
        }

        $this->asnFile = $mmdbFile['asn'];
        $this->cityFile = $mmdbFile['city'];
    }

    /**
    * Query ip info , use maxmind(geolite2) data
    */
    function get($ip = '')
    {
        $asn     = new Reader($this->asnFile); 
        $city    = new Reader($this->cityFile); 
        $asn  = $asn->get($ip);
        $city = $city->get($ip);
        return array_merge($asn,$city);

        @$ipinfo = [
            'continent' => [
                'code'  => $city['continent']['code'],
                'names' => [
                    'en'    => $city['continent']['names']['en'],
                    'zh-CN' => $city['continent']['names']['zh-CN']
                ]
            ],
            'country'   => [
                'code'  => $city['country']['iso_code'],
                'names' => [
                    'en'    => $city['country']['names']['en'],
                    'zh-CN' => $city['country']['names']['zh-CN']
                ]
            ],
            'province'  => [
                'code'  => $city['subdivisions']['0']['iso_code'],
                'names' => [
                    'en'    => $city['subdivisions']['0']['names']['en'],
                    'zh-CN' => $city['subdivisions']['0']['names']['zh-CN']
                ]
            ],
            'city'  => [
                'names' => [
                    'en'    => $city['city']['names']['en'],
                    'zh-CN' => $city['city']['names']['zh-CN']
                ]
            ],
            'location'  => [
            ...
            'registered_country'    => [
                'code'  => $city['registered_country']['iso_code'],
                'name'  => [
                    'en'    => $city['registered_country']['names']['en'],
                    'zh-CN' => $city['registered_country']['names']['zh-CN']
                ]
            ],
